                      ...
                      long double * C3_time,	long double * C6_time, long double* C7_time, struct timeval* tvstart, struct timeval* tvstop, int *max_observed_cpu)
{
    ...
    *max_observed_cpu = (socket_0.max_cpu > *max_observed_cpu)? socket_0.max_cpu: *max_observed_cpu;
    ...
    if (socket_0.max_cpu > 0) {
        ...
        {
			int offset = 19 + (socket_0.max_cpu * 2);
            mvprintw (offset, 0, "C0 = Processor running without halting");
            mvprintw (offset + 1, 0, "C1 = Processor running with halts (States >C0 are power saver)");
            mvprintw (offset + 2, 0, "C3 = Cores running with PLL turned off and core cache turned off");
            mvprintw (offset + 3, 0, "C6 = Everything in C3 + core state saved to last level cache");
            mvprintw (offset + 4, 0, "  Above values in table are in percentage over the last 1 sec");
            mvprintw (offset + 5, 0, "[core-id] refers to core-id number in /proc/cpuinfo");
            mvprintw (offset + 6, 0, "'Garbage Values' message printed when garbage values are read");
            mvprintw (offset + 7, 0, "  Ctrl+C to exit");
        }
